...
start_c_tunnel() {
    read -p "Enter tunnel name (Example: Tunnel1): " tunnel_name
    # Check if the service is installed
...
install_custom() {
    ...
    tunnel_name="Tunnel_"
    ip_address=$(echo "$arguments" | grep -oP '\b(?:\d{1,3}\.){3}\d{1,3}\b')
    tunnel_name="${tunnel_name}${ip_address}"

    # Create the custom_tunnel.service file with user input
    cat <<EOL > $tunnel_name.service
[Unit]
...
EOL
    ...
    # Reload systemctl daemon and start the service
    sudo systemctl daemon-reload
    sudo systemctl start $tunnel_name.service
    sudo systemctl enable $tunnel_name.service
    echo "This Custom Tunnel with name ($tunnel_name) was successfully installed"
}
