...
configure_arguments() {
    ...
    read -p "Activate mux? (value 2)? (yes/no): " is_mux
    ...
    if [ "$server_choice" == "2" ]; then
        if [ "$is_mux" == "yes" ]; then
            ...
            read -p "Please Enter IRAN IP(internal-server) : " server_ip
            read -p "Please Enter Password (Please choose the same password on both servers): " password
            arguments="--kharej --iran-ip:$server_ip --iran-port:443 --toip:127.0.0.1 --toport:multiport --password:$password --sni:$sni --keep-ufw --terminate:24"
        else
    ...
    if [ "$server_choice" == "2" ]; then
        ...
        if [ "$is_mux" == "yes" ]; then
            if [ "$is_main_server" == "yes" ]; then
                ...
                arguments="--kharej --iran-ip:$server_ip --iran-port:443 --toip:$main_ip --toport:multiport --password:$password --sni:$sni --keep-ufw --mux-width:2 --terminate:24"
            else
            ...
            if [ "$is_main_server" == "yes" ]; then
                ...
                read -p "Enter your main IP (VPN Server):  " main_ip
                arguments="--kharej --iran-ip:$server_ip --iran-port:443 --toip:$main_ip --toport:multiport --password:$password --sni:$sni --keep-ufw --terminate:24"
            else
                echo "Invalid choice for main server. Please enter 'yes' or 'no'."
                exit 1
            fi
...
start_c_tunnel() {
    read -p "Enter tunnel name (Example: Tunnel_1.1.1.1): " tunnel_name
    # Check if the service is installed
    if sudo systemctl is-enabled --quiet $tunnel_name.service; then
    
        sudo systemctl start $tunnel_name.service > /dev/null 2>&1

        if sudo systemctl is-active --quiet $tunnel_name.service; then
...
stop_c_tunnel() {
    ...
    if sudo systemctl is-enabled --quiet $tunnel_name.service; then
        ...
        sudo systemctl stop $tunnel_name.service > /dev/null 2>&1
...
install_custom() {
    ...
    read -p "Enter tunnel ip (Example: 1.1.1.1): " tunnel_name
    if [ -f "/etc/systemd/system/Tunnel_$tunnel_name.service" ]; then
        echo "This Custom Tunnel is already installed."
        exit 1
    fi

    # Create the custom_tunnel.service file with user input
    cat <<EOL > Tunnel_$tunnel_name.service
[Unit]
Description=Tunnel_$tunnel_name
...
EOL
    ...
    # Reload systemctl daemon and start the service
    sudo systemctl daemon-reload
    sudo systemctl start Tunnel_$tunnel_name.service
    sudo systemctl enable Tunnel_$tunnel_name.service
    echo "This Custom Tunnel with name (Tunnel_$tunnel_name) was successfully installed"
}
...
c_uninstall() {
    ...
    if [ ! -f "/etc/systemd/system/$tunnel_name.service" ]; then
        echo "This Custom Tunnel is not installed."
        return
    fi

    # Stop and disable the service
    sudo systemctl stop $tunnel_name.service
    sudo systemctl disable $tunnel_name.service

    # Remove service file
    sudo rm /etc/systemd/system/$tunnel_name.service
    sudo systemctl reset-failed
    sudo rm RTT
    sudo rm install.sh 2>/dev/null
...
check_tunnel_status
check_lb_tunnel_status
active_services=$(systemctl list-units --all | grep '^Tunnel_' | awk '{print $1}')
if [ -n "$active_services" ]; then
    echo "Active services with names starting with 'tunnel_': $active_services"
else
    echo "No active services with names starting with 'tunnel_' found."
fi
echo -e "${yellow}******************************${rest}"
